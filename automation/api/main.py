...
app.add_middleware(
    ...
    # Use specific frontend origin(s) instead of wildcard in development.
...
def list_runs(suite: Optional[str] = None, status: Optional[str] = None, limit: int = 50, db: Session = Depends(get_db)):
...
def trigger_analysis(run_id: str, background_tasks: BackgroundTasks):
    ...
    # In a real setup, this would enqueue a background job.
